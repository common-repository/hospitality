...
abstract class Hospitality_Meta_Box {
	...
	public function meta_box_init() {
		...
		add_meta_box( $this->meta_box_ID, $this->meta_box_title , array( $this, 'meta_box_render'), $this->post_type, 'normal','high' );
	...
	protected function update_meta_reference_array( $parent_post_id , $name, $fields, $validate_item = "", $post_type = 'post', $merged_fields = array(), $title_fields = array() ) {
		...
		if ( isset($_POST[ $verify_field ]) ) {
			...
			for ( $i = 0; $i < $item_count; $i ++ ) {
                ...
                $fields = $orig_fields;
				foreach ( $fields as $field ) {
                    // Merged fields are added in the validation callback. Don't look for them in the post request.
                    if ( !in_array( $field, $merged_fields ) ){
                        $item [ $field ] = sanitize_text_field( $_POST[ $name . '-' . $field ][ $i ] );
                    }
				}
			...
			foreach ( $updates as $update ){
				...
				if ( empty( $post_id ) ) {
                    ...
                    $title = "";
                    $tf_count = count( $title_fields );
                    if ( $tf_count  > 0 ) {
                        for ( $i = 0 ; $i < $tf_count ; $i++ ) {
                           $title .= $update[ $title_fields[ $i ] ];
                           if ( $i < $tf_count - 1 ) {
                               $title .= ' ';
                           }
                        }
                        ...
                        $title = $post_type . '-' . date("Y-m-d-i-s-u");
                    }
